...
def get_problem_kwargs(**main_kwargs):
    ...
    kwargs_list = (get_vtk_tetra_kwargs(), get_ecoli_kwargs(), get_forcefree_kwargs(),
                   get_integrate_kwargs(), main_kwargs,)
...
def print_case_info(caseIntro='-->Ecoli in pipe case, force free case.',
    ...
    print_integrate_kwargs(**problem_kwargs)
...
def shear_sovle_velocity_1step(ecoli_comp: 'sf.ForceFreeComposite',
    ...
    if np.all(np.isclose(planeShearRate, np.zeros(3))):
        ...
        ecoli_comp.set_rel_U_list([np.zeros(6), np.zeros(6)])  # a passive ecoli
        ...
        ecoli_comp.set_rel_U_list(tmp_rel_U_list)  # a active ecoli
    ...
    PETSc.Sys.Print(
        '    |ref_U| %s, %s' % (str(np.linalg.norm(ref_U[:3])), str(np.linalg.norm(ref_U[3:]))))
    ...
    sumF = np.sum(
        np.vstack([tobj.get_total_force(center=center) for tobj in ecoli_comp.get_obj_list()]),
        axis=0)
