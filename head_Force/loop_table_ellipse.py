...
def get_problem_kwargs(**main_kwargs):
    OptDB = PETSc.Options()
    fileHandle = OptDB.getString('f', 'loop_table_ellipse')
...
def do_solve_once(problem_ff: sf.ShearFlowForceFreeProblem,
                  ...
                  fileHandle, norm_theta, norm_phi, planeShearRate, rank, idx, N, iter_tor):
    PETSc.Sys.Print()
    PETSc.Sys.Print('%s %05d / %05d theta=%f, phi=%f, %s' %
                    ('#' * 25, idx, N, norm_theta, norm_phi, '#' * 25,))
    # 1) ini guess
...
def main_fun(**main_kwargs):
    ...
    if not problem_kwargs['restart']:
        # create helix
        ellipse_obj, _ = createEcoli_ellipse(name='ecoli0', **problem_kwargs)
        t_norm = ellipse_obj.get_u_geo().get_geo_norm()
        t_center = ellipse_obj.get_u_geo().get_center()
        ellipse_obj.move(-t_center)
        ellipse_comp = sf.ForceFreeComposite(center=np.zeros(3), norm=t_norm, name='helix_0')
        ellipse_comp.add_obj(obj=ellipse_obj, rel_U=np.zeros(6))
        problem_ff = sf.ShearFlowForceFreeProblem(**problem_kwargs)
        problem_ff.add_obj(ellipse_comp)
        problem_ff.print_info()
        problem_ff.create_matrix()
        problem = sf.ShearFlowForceFreeIterateProblem(**problem_kwargs)
        problem.add_obj(ellipse_comp)
        problem.set_iterate_comp(ellipse_comp)
        planeShearRate = problem_ff.get_planeShearRate()

        # 1). theta=0, ecoli_norm=(0, 0, 1)
        norm_theta, norm_phi = 0, 0
        t2 = time()
        do_solve_once(problem_ff, problem, ellipse_comp, fileHandle, norm_theta, norm_phi,
                      planeShearRate, rank, 0, N, iter_tor)
        ref_U000 = ellipse_comp.get_ref_U().copy()
        t3 = time()
        ...
        for i0, norm_theta in enumerate(np.linspace(0, 2 * np.pi, n_norm_theta)):
            ...
            for i1, norm_phi in enumerate(np.linspace(0, 2 * np.pi, n_norm_phi)):
                ...
                ellipse_comp.node_rotation(np.array((0, 0, 1)), -norm_phi)  # rotate back
                t3 = time()
                PETSc.Sys.Print('    Current process uses: %07.3fs' % (t3 - t2))
            ellipse_comp.node_rotation(np.array((0, 1, 0)), -norm_theta)  # rotate back
    else:
        pass
    return True
    ...
    if not problem_kwargs['restart']:
        ...
        for i0, norm_theta in enumerate(np.linspace(0, 2 * np.pi, n_norm_theta)):
            ...
            for i1, norm_phi in enumerate(np.linspace(0, 2 * np.pi, n_norm_phi)):
                ...
                idx = i0 * n_norm_phi + i1 + 1
                ellipse_comp.node_rotation(np.array((0, 0, 1)), norm_phi)
                do_solve_once_noIter(problem_ff, ellipse_comp, fileHandle, norm_theta, norm_phi,
                                     planeShearRate, rank, idx, N, iter_tor)
                ellipse_comp.node_rotation(np.array((0, 0, 1)), -norm_phi)  # rotate back
                t3 = time()
                PETSc.Sys.Print('    Current process uses: %07.3fs' % (t3 - t2))
