from src import stokesTwoPlate_base_integral as sk
from src import stokesTwoPlate_base_series as sks
...
class tank():
    ...
    def get_ufunc_series(self, point_loc, n_f=50, n_s=1):
        ...
        uabH0 = sks.np.array([sks.mp.im(pi * zm(n + 1) / H * hankel0[n] * (1 / zm(n + 1) * shsh[n] + chsh[n] + shch[n]
                                                                           - inverseOfzm_Min[n] * zm(n + 1) * (
                                                                           (x3 + h) / H * shsh[n]
                                                                           + x3 * h / H ** 2 * (
                                                                           sks.mp.cosh((h - x3) * zm(n + 1) / H) -
                                                                           chMinsh[n]))))
                              for n in range(n_f)])
        ...
        ua3H1 = sks.np.array([-pi / H * sks.mp.im(zm(n + 1) * hankel1[n] * inverseOfzm_Min[n]
                                                  * (x3 * h * zm(n + 1) / H ** 2 * (
        sks.mp.sinh((x3 - h) * zm(n + 1) / H) + shMinch[n])
        ...
        u33H0 = sks.np.array([-pi / H * sks.mp.im(zm(n + 1) * hankel0[n] * inverseOfzm_Min[n]
                                                  * (zm_Add[n] * (chsh[n] + shch[n] - 1 / zm(n + 1) * shsh[n])
                                                     - (h * x3) / (H ** 2) * zm(n + 1) * (
                                                     chMinsh[n] + sks.mp.cosh((x3 - h) * zm(n + 1) / H))
