...
full_path = os.path.normpath(t_path + '/' + 'zm_list.npy')
zm_list = np.load(full_path)
...
def uabHankel0(r, x3, h, H, n):
    return mp.im(pi * zm(n) / H * mp.hankel1(0, r * zm(n) / H) * (
        1 / zm(n) * shsh(x3, h, H, n) + chsh(x3, h, H, n) + shch(x3, h, H, n)
...
def uabHankel1(r, x3, h, H, n):
    return mp.im(
            pi * mp.hankel1(1, r * zm(n) / H) * (1 / zm(n) * shsh(x3, h, H, n) + chsh(x3, h, H, n) + shch(x3, h, H, n)
                                                 - inverseOfzm_Min(n) * zm(n) * ((x3 + h) / H * shsh(x3, h, H, n)
                                                                                 + x3 * h / H ** 2 * (
                                                                                     cosh((h - x3) * zm(
                                                                                             n) / H) - chMinsh(x3, h,
...
def ua3Hankel(r, x3, h, H, n):
    return -pi / H * mp.im(zm(n) * mp.hankel1(1, r * zm(n) / H) * inverseOfzm_Min(n) * (x3 * h * zm(n) / H ** 2
                                                                                        * (sinh(
            (x3 - h) * zm(n) / H) + shMinch(x3, h, H, n))
                                                                                        + zm(n) * (
                                                                                            chsh(x3, h, H, n) - shch(x3,
                                                                                                                     h,
                                                                                                                     H,
                                                                                                                     n))
                                                                                        + shsh(x3, h, H, n) * (
...
def u12(r1, r2, x3, h, H, n_f, n_s=1):
    ...
    return (mp.nsum(lambda n: (r1 * r2 / r ** 2 * (uabHankel0(r, x3, h, H, n) + uabK0(r, x3, h, H, n))
                               + (-2 * r1 * r2) / r ** 3 * (uabHankel1(r, x3, h, H, n) + uabK1(r, x3, h, H, n)
                                                            + r * uabK0(r, x3, h, H, n))), [int(n_s), int(n_f)]) + (
                2 * r1 * r2) / r ** 4 * uabSimple(r, x3, h, H))
