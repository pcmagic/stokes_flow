...
def get_problem_kwargs(**main_kwargs):
    OptDB = PETSc.Options()
    fileHeadle = OptDB.getString('f', 'try_singleEcoliPro2')
    nth = OptDB.getInt('nth', 10)  # amount of nodes on each cycle of helix
    ...
    rh2 = OptDB.getReal('rh2', 0.01)  # radius of helix
    ph = OptDB.getReal('ph', 1.5)  # helix pitch
    with_cover = OptDB.getBool('with_cover', True)
    left_hand = OptDB.getBool('left_hand', True)
    # rs = OptDB.getReal('rs', 0.5)  # radius of head
    rs1 = OptDB.getReal('rs1', 0.5)  # radius of head
    rs2 = OptDB.getReal('rs2', 1)  # radius of head
    ds = OptDB.getReal('ds', 0.08)  # delta length of sphere
